module mokuai {
    //三角形
    export class SJX extends XingZhuang {
        constructor(scene: scene.TestScene, posX: number, posY: number, sf: number, mass: number, type: number, peise: any, pz_numb: number, k?: any) {
            if (k) {
                super(scene, "sanjiao_k", posX, posY, sf, mass, type, peise, pz_numb);
            } else
                super(scene, "sanjiao", posX, posY, sf, mass, type, peise, pz_numb);
            this.initShape();
            this.initPost();
            this.initColl();
            this.lx = 9;
