module mokuai {
    export class YXSJ extends XingZhuang {
        constructor(scene: scene.TestScene,posX: number, posY: number, sf: number, mass: number, type: number, peise: any,pz_numb:number, k?: any) {
            if (k)
                super(scene,"youxiesanjiao_k", posX, posY, sf, mass, type, peise,pz_numb);
            else
                super(scene,"youxiesanjiao", posX, posY, sf, mass, type, peise,pz_numb);
            this.initShape();
            this.initPost();
            this.initColl();
            this.lx = 4;
